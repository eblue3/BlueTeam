...
.data
pinfo				PLUGIN_INFO <DUP2_PLUGIN_VERSION,"com.d2k2.delete_file","[Delete File]","delete_file.d2p">
...
DUP2_ModuleDescription proc uses esi edi ebx _plugin_data:DWORD
	...
	lea eax,[esi].MY_PLUGIN_DATA_STRUCTURE.filepath
...
DialogPlugin proc uses ebx esi edi hwnd,message,wparam,lparam
	...
	.elseif eax==WM_COMMAND
		...
		.elseif ax==BTN_SAVE
			...
			fn GetDlgItemText,hDialogPlugin,DLG_FILE,addr local_buffer,sizeof local_buffer
			.if eax > 0
				;---save settings before close---
				fn SavePluginData
				fn SendMessage,hwnd,WM_CLOSE,0,0
			.else
				fn MessageBox,hwnd,"No Filename!",0,MB_ICONEXCLAMATION
			.endif	
		.endif
...
SavePluginData proc uses esi edi ebx
	...
	;---filename---
	fn GetDlgItemText,hDialogPlugin,DLG_FILE,addr [esi].MY_PLUGIN_DATA_STRUCTURE.filepath,1024
...
LoadPluginData proc uses esi edi ebx
	...
	;---filename---
	fn SetDlgItemText,hDialogPlugin,DLG_FILE,addr [esi].MY_PLUGIN_DATA_STRUCTURE.filepath
