...
.data
pinfo				PLUGIN_INFO <DUP2_PLUGIN_VERSION,"com.d2k2.filetime","[Filetime]","filetime.d2p">
...
DUP2_ModuleDescription proc uses esi edi ebx _plugin_data:DWORD
	...
	.if [esi].MY_PLUGIN_DATA_STRUCTURE.options & FT_GET_FILETIME
		mov ebx,chr$("GetFiletime")
	.else
		mov ebx,chr$("SetFiletime")
	.endif
...
DialogPlugin proc uses ebx esi edi hwnd,message,wparam,lparam
	...
	.if eax==WM_INITDIALOG
		...
		mov hDROPDOWN_TYPE,eax
	        fn SendMessage,hDROPDOWN_TYPE,CB_ADDSTRING,0,"Get Filetime"
	        fn SendMessage,hDROPDOWN_TYPE,CB_ADDSTRING,0,"Set Filetime"
	        fn SendMessage,hDROPDOWN_TYPE,CB_SETCURSEL,0,0
      		
      		
      		;---tooltip---
      		fn AddToolTip,hwnd,DLG_VAR_NAME,"Enter here the name of the variable where the filetime will be stored!"
	...
	.elseif eax==WM_COMMAND
		...
		.elseif ax==BTN_SAVE
			...
			.if eax
				...
				fn SendMessage,hwnd,WM_CLOSE,0,0
			.else
				fn MessageBox,hDialogPlugin,"Please enter a variable name!",0,MB_ICONEXCLAMATION
			.endif
