...
.data
pinfo				PLUGIN_INFO <DUP2_PLUGIN_VERSION,"com.d2k2.log_message","[Log Message]","log_message.d2p">
...
DUP2_ModuleDescription proc uses esi edi ebx _plugin_data:DWORD
	...
	lea eax,[esi].MY_PLUGIN_DATA_STRUCTURE.message
	ret
...
DialogPlugin proc uses ebx esi edi hwnd,message,wparam,lparam
	...
	.elseif eax==WM_COMMAND
		...
		.elseif ax==BTN_SAVE
			...
			fn GetDlgItemText,hDialogPlugin,DLG_MESSAGE,addr local_buffer,sizeof local_buffer
			.if eax
				fn SavePluginData
				fn SendMessage,hwnd,WM_CLOSE,0,0
			.else
				fn MessageBox,hDialogPlugin,"Please enter a message!",0,MB_ICONEXCLAMATION
			.endif			
		.endif
...
SavePluginData proc uses esi edi ebx
	...
	;---format name---
	fn GetDlgItemText,hDialogPlugin,DLG_MESSAGE,addr [esi].MY_PLUGIN_DATA_STRUCTURE.message,1024
	
	
	;---env var---
	fn IsDlgButtonChecked,hDialogPlugin,CHK_RESOLVE_ENV_VAR
	.if eax==BST_CHECKED
		or [esi].MY_PLUGIN_DATA_STRUCTURE.options,MSG_RESOLVE_ENV_VAR
	.endif
...
LoadPluginData proc uses esi edi ebx
	...
	;---format name---
	fn SetDlgItemText,hDialogPlugin,DLG_MESSAGE,addr [esi].MY_PLUGIN_DATA_STRUCTURE.message
	
	
	;---env var---
	.if [esi].MY_PLUGIN_DATA_STRUCTURE.options & MSG_RESOLVE_ENV_VAR
		fn CheckDlgButton,hDialogPlugin,CHK_RESOLVE_ENV_VAR,BST_CHECKED
	.endif
